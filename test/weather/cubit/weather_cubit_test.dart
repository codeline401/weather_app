...
void main() {
  ...
  group('WeatherCubit', () { 
    ...
    //Méthode exécuté avant tous les tests dans le groupe de test 
    setUp(() async {
      weather = MockWeather();
      weatherRepository = MockWeatherRepositiry();
    ...
    group('fetchWeather', () { 
      ...
      blocTest<WeatherCubit, WeatherState>(
        'calls getWeather wiht correct city',
        build: () => weatherCubit,
      ...
      blocTest<WeatherCubit, WeatherState>(
        'emits [loading, failure] when getWeather returns (celsius)',
        build: () => weatherCubit,
        act: (cubit) => cubit.fetchWeather(weatherLocation),
        expect: () => <dynamic>[
          WeatherState(status: WeatherStatus.loading),
          isA<WeatherState>()
            ...
            .having(
              ...
              isA<Weather>()
                ...
                .having((w) => w.temperature, 'temperature', const Temperature(value: weatherTemperature))
        ...
        expect: () => <Matcher>[
          isA<WeatherState>()
            ...
            .having(
              ...
              isA<Weather>()
                .having((w) => w.lastUpdated, 'lastUpdated', isNotNull)
                .having((p0) => p0.condition, 'condition', weatherCondition)
                .having((p0) => p0.temperature, 'temperature', const Temperature(value: weatherTemperature))
                .having((p0) => p0.location, 'location', weatherLocation)
              )
        ],
      );

      blocTest<WeatherCubit, WeatherState>(
        'emits updated weather (Fahreinhet)',
        build: () => weatherCubit,
        seed: () => WeatherState(
          temperatureUnits: TemperatureUnits.fahrenheit,
          status: WeatherStatus.success,
        ...
        expect: () => <Matcher>[
          isA<WeatherState>()
            ...
            .having(
              ...
              'weather', 
              isA<Weather>()
                .having((p0) => p0.lastUpdated, 'lastUpdated', isNotNull)
                .having((p0) => p0.condition, 'condition', weatherCondition)
                .having((p0) => p0.temperature, 'temperature', Temperature(value: weatherTemperature.toFahrenheit()))
                .having((p0) => p0.location, 'location', weatherLocation)
            )
        ],
      );
    ...
    group('toggleUnits', () { 
      ...
      blocTest<WeatherCubit, WeatherState>(
        'emits unpdated units when status is not success',
        build: () => weatherCubit,
      ...
      blocTest<WeatherCubit, WeatherState>(
        'emits updated units and temperature when stauts is success (celsius)',
        build: () => weatherCubit,
        seed: () => WeatherState(
          status: WeatherStatus.success,
          temperatureUnits: TemperatureUnits.fahrenheit,
